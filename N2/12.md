# 复合句-定语从句

* 定语从句的含义？

一个句子作定语去修饰和限定名词

* 定语从句的位置？

* 定语从句的写法？

This is `the job`.

I have dreamed of `the job` for years.
* `the job` -> which(关系词，引导作用)

This is `the job` `which I have dreamed of for years`.
* `the job`: 先行词，定语从句所修饰的名词
* which：关系词


关系词做不做成分？

**定语从句中，关系词一定要做成分**

**先行词 = 关系词**

### 定语从句的写法？

先行词n. + (关系词+陈述句)=定语从句

`关系词选用`看`先行词`

先行词|关系词
-|-
事/物|which/that
人|who/whom(指人只能做宾语)/that
人/物<br>("某人的/某物的")|whose
时间|when
地点|where
原因|why

**关系词的几点注意：**

* 定语从句的关系词每一个都作成分，包括 that
* whom 指人，只能在作宾语时使用
* when/where/why 引导定语从句，必须与先行词匹配
* 关系词在定语从句中作宾语时，可以省略

I have a class [which] `begins at 8:00 am`.
* `a class` -> 先行词是(事/物),所以选择 which
* `which` 省略？-> 不能,`begins`表名它作主语，主语不能省，宾语才能省

The lawyer [省略/whom/who/that] `my brother called` didn't answer the phone. 我哥哥打电话的那个律师没有接电话
* `The lawyer`->先行词是人
* 那个律师，那个律师-->我哥哥打电话的那个
* 关系词明显做宾语，所以可以用 `whom`
* 关系词多样化(能省则省，用专用)，它们的优先级是
  * 省略
  * whom(先行词是`人`然后`关系词`明显`做宾语`),专用
  * who/that

My daughter asked me a question [省略/which/that] I couldn't answer.
* `question` 先行词是物 -> 关系词用 which/that
* 从句中关系词作宾语 -> 关系词可省略

Leo is the student `whose` bike was stolen.
* 人的车 -> ("某人的/某物的")
* 这里 `whose` 它做宾语不能省略

Australia is one of the few countries `where` people drive on the left.
* 地点 where

Sunday is the day `when` people usually don't go to work
* 时间 when

Is there any particular reason `` you can't come?
* 原因 why

**定语从句的写法**

1）I lost the book `(which/that/还可以省略) I borrowed from the library last week`.
* 限定的是书📚

我弄丢了上周从图书馆借的书。

2) The woman `(who/that/whom/还可以省略) I saw in the park` was feeding pi

我在公园看见那位女士正在喂鸽子。

3）We like Profeesor Lee `whose lecture is very amazing`.
* 他的讲座->> whose

我们喜欢李教授，他的讲座非常精彩。

4) I cannot remember the day `when we met last time`.

我记不起上一次我们是什么时候碰面的。

### 定语从句的分类？

**限定** 与 **非限定** 取决于 **先行词的范围是否明确**

He will call `his friend` who is woking in London.
* 他会打给他的朋友
* `his friend` 范围不明确，用句子限定一下范围
* 限定性定语从句，因为它所修饰的名词范围不确定

He will call `his mother`, who is working in London.
* `his mother`->妈就一个，所以不需要限定
* 所以后面的从句就是 `非限定性定语从句`, 所以用逗号隔开

**专有的，唯一的，上下文明确的名词：**--`非限定性定语从句`

`非限定性定语从句` 关系词关系词比较严格
* 东西 -> which
* 人 -> who
* 指人做宾语 -> whom
* 不能有 that

He came from Beijing `,which` is the capital of China.
* 非限定用 `which`
* 这个考察的是定语从句中关系词的选择，如果先行词在定语从句中做的是主语或者宾语，就得用关系代词that或者which，如果是先行词在从句中做状语就得用关系副词where。这里先行词在从句中做的是主语，所以选which不选where。

He came from a city `which` is in the north of China.
* `a city` 不明确，限定

This is the businessman `who` we are cooperating with.
* 这是我们正在合作的那个商人
* `businessman`，不明确，要限定
* 从句中作宾语, 所以（who/that/whom/省略）

This is `Bill Gates` `,whom` many people know(...) as the richest man.许多人知道
* `Bill Gates`(人) -> 明确的名词 -> 用非限定
* 从句中作宾语 -> 只能用 `whom`

**Lesson 28: 限定性定语从句**

**Lesson 36：非限定性定语从句**

### Lesson 28: No parking

1. Jasper White is one of those rare people `who believes in ancient myths`.
* rare people -> 少数人
* believes in -> 信仰
* 限定 `people`

2. He `has just bought` a new house in the city, but ever since he moved in, he `has had` trouble with cars and their owners.
* have done(现在完成时)
* has had ->> (have`有` 的现在完成时，现在的之前有)

3.`When he returns home at night`, he always finds `that someone has parked a car outside his gate`.
* 时间状语从句
* 宾语从句

4.`Because of this`, he has not been able to get his own car into his garage even once.
* (have+done) + (be able to)--->>he has not been able to
* `because` 是连词，后面加句子
* `because of` 是介词词组，后面加词

Men always remember love because of romance only.人们总是能够记住爱情仅仅因为浪漫。

Men always remember love because of romance over.人们总是能够记住爱情因为浪漫结束了。

Marlboro|Marlboro
-|-
Men|Men
always|always
remember|remember
love|love
because|because
of|of
romance|romance
only|over

Father and mother,I love you! ----> family

5. It is one of the ugliest faces `I have ever seen`. 是我曾经见过的最丑的脸之一
* `I have ever seen` 定语从句修饰 `faces`

6. I asked him `What it was` and he told me `that it was Medusa, the Gorgon`.
* ask sb. sth. 双宾语结构，宾语从句
* tell sb. sth. 双宾语结构，宾语从句

7. Jasper hopes `that she will turn cars and their owners to stone`.
* turn A to B ：把 A 变成 B
* turn A into B：把 A 变成 B

8. But none of them has been turned to stone yet!
* has been turned 现在完成时的被动

### Lesson 36: Across the Channel

intend to do sth. 打算做某事

1.Debbie Hart is going to swim across the English Channel tomorrow.
* be going to 将来时

2. She is going to set out from the French coast at five o'clock in the morning.

3. Tomorrow he will be watching her anxiously `as she swims the long distance to England`.
* 将来进行时
* as...  当....时候--> 引出时间状语从句
* 条件和时间状语从句，用一般现在时

4. Debbie `intends to take short rests` every two hours.

5. `Most of Debbie's school friends` will be waiting for her on the English coast.

6. Among `them` will be `Debbie's mother`, `who` swam the Channel herself when she was a girl.
* `Debbie's mother`, 明确，非限定从句
* Among `them` will be `Debbie's mother` -> 倒装句，承上启下
  * 介词短语不可能做主语
  * 正常的应该是：`Debbie's mother` will be Among `them`
